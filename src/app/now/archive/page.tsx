...
export default function NowArchivePage() {
  const [archiveData, setArchiveData] = useState<NowArchiveContent | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const fetchArchive = async () => {
      try {
        const response = await fetch('/api/now-archive')
        if (response.ok) {
          const data = await response.json()
          setArchiveData(data)
        }
      } catch (error) {
        console.error('Error fetching now archive:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 ambient:ambient-gradient-bg ambient:dark:ambient-gradient-bg-dark transition-colors duration-300 flex items-center justify-center relative">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="text-slate-600 dark:text-slate-400">Loading archive...</p>
        </div>
      </div>
    )
  }
  ...
  if (!archiveData) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 ambient:ambient-gradient-bg ambient:dark:ambient-gradient-bg-dark transition-colors duration-300 flex items-center justify-center relative">
        <AnimatedBackground />
        <div className="text-center relative z-10">
          <p className="text-slate-600 dark:text-slate-400">Failed to load Now archive.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 ambient:ambient-gradient-bg ambient:dark:ambient-gradient-bg-dark transition-colors duration-300 relative">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center gap-4">
              <Link
                href="/now"
                className="flex items-center gap-3 text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors"
              >
                ...
                <span className="text-xl font-bold text-slate-900 dark:text-white">Back to Now</span>
              </Link>
            </div>
            <div className="flex items-center space-x-4">
              <span className="text-sm text-slate-600 dark:text-slate-400 font-medium">Now Archive</span>
              <ThemeToggle />
            </div>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 max-w-xl mx-auto transition-colors mb-6">
            {archiveData.intro}
          </p>
          <Button asChild variant="outline" size="sm" className="dark:border-slate-600 dark:text-slate-300">
            <Link href="/now">View current Now</Link>
          </Button>
        </div>

        {archiveData.entries.length === 0 ? (
          <Card className="dark:bg-slate-800 dark:border-slate-700 ambient:glass-card ambient:dark:glass-card-dark">
            <CardContent className="py-12 text-center">
              <p className="text-slate-600 dark:text-slate-400">No archived months yet.</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-16">
            {archiveData.entries.map((entry) => (
              <section key={entry.id} aria-labelledby={`archive-${entry.id}`}>
                <div className="flex items-center gap-3 mb-6">
                  ...
                  >
                    <Badge
                      variant="outline"
                      className="dark:border-slate-600 dark:text-slate-300 text-sm px-3 py-1 font-semibold"
                    >
